...
void set_params_fprop(FLASH_NAMESPACE::Flash_fwd_params &params,
                      // sizes
                      const size_t b,
...
void run_mha_fwd(FLASH_NAMESPACE::Flash_fwd_params &params, cudaStream_t stream, bool force_split_kernel=false) {
    ...
    //             } else {
    //                 FLASH_NAMESPACE::run_mha_fwd_splitkv_dispatch<elem_type, kHeadDim, Is_causal>(params, stream);
    //             }
    //         });
    //     });
    // });

    TORCH_CHECK(params.is_bf16, "qkv.dtype must be torch.bfloat16")
    using elem_type = cutlass::bfloat16_t;
    TORCH_CHECK(params.d, "qkv.headdim must <= 128")
    constexpr static int kHeadDim = 128;
    // mha_fwd_kvcache: Is_causal=true <-- seqlen_q!=1 <-- prefill 
    // mha_fwd_kvcache: Is_causal=false <-- (seqlen_q==1 && !alibi_slopes_.has_value()) <-- decode 
    // mha_carlen_fwd: Is_causal=false <-- prefill 
    BOOL_SWITCH(params.is_causal, Is_causal, [&] {
        if (params.num_splits <= 1 && !force_split_kernel) {  // mha_varlen_fwd
            FLASH_NAMESPACE::run_mha_fwd_<elem_type, kHeadDim, Is_causal>(params, stream);
        } else { // mha_fwd_kvcache
            FLASH_NAMESPACE::run_mha_fwd_splitkv_dispatch<elem_type, kHeadDim, Is_causal>(params, stream);
        }
    });
...
std::tuple<at::Tensor, at::Tensor> set_params_splitkv(FLASH_NAMESPACE::Flash_fwd_params &params, const int batch_size,
...
void set_params_alibi(FLASH_NAMESPACE::Flash_fwd_params &params, std::optional<at::Tensor> &alibi_slopes_, int batch_size, int num_heads){
