...
bool is_valid_config(thread_config_t const& th_config, int thread_m_blocks,
  ...
  #define _GET_IF(W_TYPE, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS,   \
      ...
      constexpr auto S_TYPE =                                                  \
          W_TYPE == purevlm::kFE2M1f                                              \
              ? (GROUP_BLOCKS == 1 ? purevlm::kFE4M3fn : purevlm::kFE8M0fnu)         \
              : (std::is_same<scalar_t, half>::value ? purevlm::kFloat16          \
                                                     : purevlm::kBFloat16);       \
...
MarlinFuncPtr get_marlin_kernel(const purevlm::ScalarType q_type,
  ...
  COMMON_GET_IF(purevlm::kU4)
  COMMON_GET_IF(purevlm::kU4B8)
  COMMON_GET_IF(purevlm::kU8B128)

  NVFP4_GET_IF(purevlm::kFE2M1f)

  BIGGROUP_GET_IF(purevlm::kFE4M3fn)

  ACT_GET_IF(purevlm::kU4B8)
  ACT_GET_IF(purevlm::kU8B128)

  if (std::is_same<scalar_t, half>::value) {
    if (false) {
    }
    FZP_GET_IF(purevlm::kU4)
  }
  if (std::is_same<scalar_t, nv_bfloat16>::value) {
    if (false) {
    }
    MXFP4_GET_IF(purevlm::kFE2M1f)
  }
...
template <typename scalar_t>
exec_config_t determine_exec_config(const purevlm::ScalarType& q_type, int prob_m,
...
void marlin_mm(const void* A, const void* B, void* C, void* C_tmp, void* b_bias,
               ...
               void* workspace, purevlm::ScalarType const& q_type, bool has_bias,
  ...
  if (has_zp) {
    TORCH_CHECK(
        q_type == purevlm::kU4 || q_type == purevlm::kU8,
        "q_type must be u4 or u8 when has_zp = True. Got = ", q_type.str());
  } else {
    TORCH_CHECK(
        q_type == purevlm::kU4B8 || q_type == purevlm::kU8B128 ||
            q_type == purevlm::kFE4M3fn || q_type == purevlm::kFE2M1f,
...
torch::Tensor gptq_marlin_gemm(
    ...
    int64_t size_k, bool is_k_full, bool use_atomic_add, bool use_fp32_reduce,
    bool is_zp_float) {
  purevlm::ScalarType const b_q_type = purevlm::ScalarType::from_id(b_q_type_id);
  ...
  if (global_scale_or_none.has_value()) {
    ...
    TORCH_CHECK(b_q_type == purevlm::kFE2M1f && group_size == 16,
  ...
  if (has_zp) {
    TORCH_CHECK(
        b_q_type == purevlm::kU4 || b_q_type == purevlm::kU8,
        "b_q_type must be u4 or u8 when has_zp = True. Got = ", b_q_type.str());
  } else {
    TORCH_CHECK(b_q_type == purevlm::kU4B8 || b_q_type == purevlm::kU8B128 ||
                    b_q_type == purevlm::kFE4M3fn || b_q_type == purevlm::kFE2M1f,
