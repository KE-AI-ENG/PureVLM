...
template <typename scalar_t,  // compute dtype, half or nv_float16
          const purevlm::ScalarTypeId w_type_id,  // weight ScalarType id
          const purevlm::ScalarTypeId s_type_id,  // weight scale ScalarType id
...
__global__ void Marlin(
  ...
  static constexpr auto w_type = purevlm::ScalarType::from_id(w_type_id);
  static constexpr auto s_type = purevlm::ScalarType::from_id(s_type_id);
  if constexpr (w_type == purevlm::kFE2M1f) {
    static_assert(s_type == purevlm::kFE4M3fn && group_blocks == 1 ||
                  s_type == purevlm::kFE8M0fnu && group_blocks == 2);
  } else if constexpr (std::is_same<scalar_t, nv_bfloat16>::value) {
    static_assert(s_type == purevlm::kBFloat16);
  } else if constexpr (std::is_same<scalar_t, half>::value) {
    static_assert(s_type == purevlm::kFloat16);
  }

  constexpr bool has_zp = w_type == purevlm::kU4 || w_type == purevlm::kU8;
  constexpr bool is_int_type = w_type == purevlm::kU4 || w_type == purevlm::kU8 ||
                               w_type == purevlm::kU4B8 || w_type == purevlm::kU8B128;
  // see comments of dequant.h for more details
  constexpr bool dequant_skip_flop =
      w_type == purevlm::kFE4M3fn ||
      w_type == purevlm::kFE2M1f && s_type == purevlm::kFE4M3fn ||
      has_zp && !is_zp_float && !std::is_same<scalar_t, nv_bfloat16>::value ||
      has_zp && !is_zp_float && !(w_type == purevlm::kU8);

  scalar_t2 global_scale;
  if constexpr (w_type == purevlm::kFE2M1f && s_type == purevlm::kFE4M3fn) {
  ...
  constexpr int s_tb_groups =
      !has_act_order && group_blocks != -1 && group_blocks < thread_k_blocks
          ? thread_k_blocks / group_blocks / (w_type == purevlm::kFE2M1f ? 2 : 1)
          : 1;
  ...
  int s_sh_rd;
  if constexpr (group_blocks != -1 && w_type == purevlm::kFE2M1f) {
    ...
    if constexpr (!has_act_order) {
      ...
      if constexpr (group_blocks == -1) {
        ...
        if constexpr (group_blocks >= thread_k_blocks) {
          ...
          int cur_group_id =
              k_blocks / (group_blocks * (w_type == purevlm::kFE2M1f ? 2 : 1));

          int4* sh_s_stage = sh_s + s_sh_stage * pipe;

          if constexpr (w_type_id != purevlm::kFE2M1f.id()) {
