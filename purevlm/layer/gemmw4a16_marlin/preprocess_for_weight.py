...
def preprocess(
    ...
    return marlin_q_w, marlin_s, marlin_zp, g_idx, sort_indices, workspace

def run(
        a: torch.Tensor, # shape[m, k]/[1, m, k]
    ...
    if a.ndim == 3:
        assert a.shape[0] == 1
        gemm_a = a.squeeze(0)
    size_m, size_k = gemm_a.shape

    output_ = torch.ops._C.gptq_marlin_gemm(
            gemm_a,
            None,
            marlin_q_w,
            bias,
